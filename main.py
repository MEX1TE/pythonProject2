...
Base = declarative_base()  # Исправлено: используется orm.declarative_base()
...
class Order(Base):
    ...
    created_at = Column(DateTime, default=datetime.datetime.utcnow)  # Исправлено: datetime.datetime.utcnow
...
async def register(credentials: Credentials, db: Session = Depends(get_db)):
    ...
    new_user = User(username=credentials.username, password=credentials.password)
    db.add(new_user)
    db.commit()
...
async def login(credentials: Credentials, db: Session = Depends(get_db)):
    ...
    # Возвращаем простой токен (имя пользователя) для совместимости
